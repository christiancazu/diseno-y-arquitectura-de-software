<%-- 
    ...
    Created on : May 31, 2019, 4:01:13 PM
...
<%
    ...
    // get empleado to be set on formGroups inputs
    ...
    formGroups.add(
            new FormGroup("Nombre:", "nombre", empleado.getNombre(), "text", "nombre", "text-nombre", true));
    formGroups.add(
            new FormGroup("Apellido", "apellido", empleado.getApellido(), "text", "apellido", "text-apellido", true));
    formGroups.add(
            new FormGroup("Login", "login", empleado.getLogin(), "text", "login", "text-login", true));
...
<t:baseTemplate
    pageTitle="Actualizar empleados"
...
<t:mainContentTemplate>
    ...
    <t:mainCardTemplate cardTitle="Actualizar empleado">
        <form action="actualizarEmpleado" method="POST">                       

            <input id="id-empleado" type="hidden" name="id" value="${empleado.id}">
...
<script>
    ...
    $(document).ready(() => {
        if (!window.location.search) {
            ...
            window.history.replaceState({}, "", baseUrl + "?id=" + $('#id-empleado').val())
        }    
    }) 
</script>